...
2. runs the real binary;
3. sources the file if the binary wrote to it;
4. deletes it and returns the binary's exit status.

And what the binary does, on the first switch in a shell that has the hook: copy the merged kubeconfig into `$XDG_STATE_HOME/kube-ctx/shells/<id>.yaml`, point `current-context` at the target, and write `export KUBECONFIG=…` into the env file. The global kubeconfig is left untouched. Later switches in that shell land in the copy directly, because `$KUBECONFIG` already points there.
...
| `SHELL` | ✅ | — | Which shell to spawn, and the default for `kctx init` |
